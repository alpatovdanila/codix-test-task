...
export const CarRow = ({ car, onDelete }: CarCardProps) => {
  return (
    <div className={cn(previewStyles.CarPreview, rowStyles.CarRow)}>
      ...
      <div className={cn(rowStyles.Cell, rowStyles.Price)}>
        <Price price={car.price} />
      </div>
      <div className={rowStyles.Cell}>{car.year}</div>
      <div className={rowStyles.Cell}>{getStatusLabel(car.status)}</div>
      <div className={rowStyles.Cell}>
        <button className={previewStyles.HoverButton} onClick={onDelete}>
          Удалить
        </button>
      </div>
    </div>
